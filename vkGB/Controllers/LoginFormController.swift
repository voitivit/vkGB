...
class LoginFormController: UIViewController, UITextFieldDelegate {
    ...
    //кнопка входа с помощью Вконтакте
    ...
    func animate() {
        ...
        UIView.animate(withDuration: 1.5, animations: {
            self.loginVK.frame.origin.y += 100
        } ,  completion: { _ in
            UIView.animate(withDuration: 1.0) {
                self.loginVK.backgroundColor = .blue
            }
        })
    }
...
// авторизация
        guard loginTextField.text == "admin" && passwordTextField.text == "123" else {
